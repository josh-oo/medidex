...
def _build_backend_url(path: str) -> str:
    base_url = os.getenv("BACKEND_API_URL", "").strip()
    if not base_url:
        raise ValueError("BACKEND_API_URL is not set")
    return f"{base_url.rstrip('/')}/api{path}"


def _build_backend_headers(accept: str) -> dict[str, str]:
...
def fetch_report_pdf(report_id: int) -> bytes | None:
    url = _build_backend_url(f"/reports/{report_id}/pdf")
    headers = _build_backend_headers("application/pdf")
    request = urllib.request.Request(url, headers=headers, method="GET")
    timeout_seconds = float(os.getenv("BACKEND_TIMEOUT_SECONDS", "60"))
    max_retries = int(os.getenv("BACKEND_PDF_RETRIES", "2"))
    base_backoff = float(os.getenv("BACKEND_PDF_RETRY_BACKOFF_SECONDS", "0.5"))
