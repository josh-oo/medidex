...
async def check_report_access(
    ...
    if project_id not in assigned_project_ids:
        raise HTTPException(
            status_code=403,
            detail="You can only modify reports in projects assigned to you",
        )
...
async def similar_tags_by_report(report_id: int, tag_category: TagCategories =Query(TagCategories.default), sources: List[str] = Query(..., description="Which source of tags do you want to search ('mesh', 'internal' or both)"), k : int = k_query, tag_similarity_service : TagSimilaritySearchService = Depends(get_tag_similarity_service)) -> List[Tag]:
...
async def similar_tags(tag_category: TagCategories =Path(..., description="The tags category (e.g. 'interventions', 'conditions', ...)"), tag_value : str = Path(..., description="The specific tags value (e.g. 'Placebo' for interventions)"), sources: List[str] = Query(..., description="Which source of tags do you want to search ('mesh', 'internal' or both)"), k : int = k_query, tag_similarity_service : TagSimilaritySearchService = Depends(get_tag_similarity_service)) -> List[Tag]:
    if tag_category == TagCategories.default:
        raise HTTPException(status_code=400, detail="No tags for 'default' embedding.")
