...
def get_document_service(report_id : int, report_repo : ReportRepository = Depends(get_report_repo)) -> DocumentService:
    return DocumentService(report_id=report_id, report_repo=report_repo)

def get_report_service(report_id : int, report_repo : ReportRepository = Depends(get_report_repo), study_repo : StudyRepository = Depends(get_study_repo),document_service : DocumentService = Depends(get_document_service)) -> ReportService:
    return ReportService(report_id=report_id, report_repo=report_repo, study_repo=study_repo, document_service=document_service)
