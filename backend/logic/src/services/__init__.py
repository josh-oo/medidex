...
def get_study_similarity_service(user_id : str = Depends(get_user_id), study_repo : StudyRepository = Depends(get_study_repo),report_repo : ReportRepository = Depends(get_report_repo), batch_repo : BatchRepository = Depends(get_batch_repo), author_feature_service : AuthorFeatureService = Depends(get_author_feature_service), vectorstore : VectorstoreService = Depends(get_vectorstore_service)) -> StudySimilaritySearchService:
    return StudySimilaritySearchService(user_id=user_id, vectorstore=vectorstore, study_repo=study_repo, report_repo=report_repo, batch_repo=batch_repo, author_feature_service=author_feature_service)
