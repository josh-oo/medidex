...
def create_test_set(path, cutoff, model_id):
    ...
    while True:
        ...
        response = requests.get(f"http://{DATABASE_HOST}:{DATABASE_PORT}/reports/{report_id}")
        ...
        response = requests.post(BACKEND_API + f"/api/v1/analyze_embedding",headers=headers, json=payload, params=params) 
